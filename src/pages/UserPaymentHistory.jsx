...
const UserPaymentHistory = () => {
    ...
    const fetchRechargeData = async (referenceId) => {
        try {
            const response = await axios.get(`http://localhost:8080/userGame/getRecharge?referanceId=${referenceId}`);
    ...
    const fetchWithdrawData = async (referenceId) => {
        try {
            const response = await axios.get(`http://localhost:8080/userGame/getWithdraw?referanceId=${referenceId}`);
            if (response.data && response.data.message === "success") {
